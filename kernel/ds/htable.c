...
{
	int i;
	struct hash_table *table = (struct hash_table*)kmalloc(sizeof(struct hash_table));
	if ( table == NULL ) return NULL;
	  ...
	  table->table = (void**)kmalloc(table->size * sizeof(void**));
...
int htable_rehash( struct hash_table* table, int new_size )
{
	...
	void **newTable = (void**)kmalloc(sizeof(void**) * new_size);
