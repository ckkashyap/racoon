...
/** Destroy a page directory regardless of the contents
 * or reasons of the map.
 */
void 	  delete_page_directory( uint32_t* map )
{
 	...
 	for ( i = 512; i < 1016; i++)
	{
		...
		if ( page_table_present( map, position ) != 0 )
				continue;

		if ( page_entry_present( map, position ) != 0 )
				continue;

		page = page_entry( map, position );
		
		table = page_table( map, position );
		table[ page_entry_of(position) ] = 0;
...
int			page_direct_map( uint32_t* directory, 
							 uint32_t physical, 
							 uint32_t start, 
							 int pages, 
							 unsigned int type )
