...
void pci_read_bases( struct pci_cfg *cfg, int tot_bases, int rom )
{
  ...
  kmemset( cfg->base, 0, sizeof( cfg->base ) );
  kmemset( cfg->size, 0, sizeof( cfg->size ) );
  kmemset( cfg->type, 0, sizeof( cfg->type ) );
				  ...
				  struct pci_cfg *cfg )
{
  ...
  while ( tmp != NULL )
  {
    ...
    if ( (act_cfg->vendor_id == vendor_id) && (act_cfg->device_id == device_id) )
		if ( count++ == num )
	    {
	      kmemcpy( cfg, act_cfg, sizeof( struct pci_cfg ) );
	      return 0;
	    }
    ...
    for ( dev = 0; dev < 32; dev++ )
      for ( func = 0; func < 8; func++ )
        if ( pci_probe( bus, dev, func, pcfg ) == 0 )
        {
		  ...
		  pcfg = (struct pci_cfg*)kmalloc( sizeof( struct pci_cfg ) );
		  assert( pcfg != NULL );

          list_item = (struct pci_list_t*)kmalloc( sizeof( struct pci_list_t ) );
