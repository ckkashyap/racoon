...
{
	return _sysenter( (SYS_IRQ|SYS_TWO), irq_num, 0, 0, 0,0 );
}


int smk_ack_irq( int irq )
{
	return _sysenter( (SYS_IRQ|SYS_FOUR), irq, 0, 0,0,0 );
}


int smk_signal_irq_complete( int status )
{
	return _sysenter( (SYS_IRQ|SYS_THREE), status, 0, 0,0,0 );
}
...
void smk_irq_stub( irq_func fn, int irq_num, void *data )
{
	...
	{
		rc = fn( irq_num, data );
		if ( smk_signal_irq_complete( rc ) != 0 ) break;
	}
}
